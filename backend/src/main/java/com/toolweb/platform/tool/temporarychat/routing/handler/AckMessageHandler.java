...
@Component
public class AckMessageHandler implements ChatFrameHandler {
    private final TemporaryChatRealtimeService realtime;
    public AckMessageHandler(TemporaryChatRealtimeService realtime) { this.realtime = realtime; }
    @Override public ChatMessageType type() { return ChatMessageType.ACK; }
    @Override public void handle(WebSocketSession socket, ChatFrame frame) { realtime.acknowledge(socket, frame); }
}
