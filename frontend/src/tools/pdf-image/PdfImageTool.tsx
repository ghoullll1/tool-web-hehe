...
export default function PdfImageTool({ tool }: ToolViewProps) {
  ...
  return <div className="pic-tool" data-tool={tool.slug}>
    ...
    <div className="pic-workspace"><section className="pic-files"><header><h3>01 {mode === 'pdf' ? '选择 PDF' : '添加图片'}</h3><button disabled={locked || !sources.length} onClick={() => reset()}>清空</button></header>
      ...
      <button className={`pic-drop ${dragging ? 'is-dragging' : ''}`} disabled={locked} onClick={() => input.current?.click()} onDragOver={(event) => { event.preventDefault(); setDragging(true) }} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); add(Array.from(event.dataTransfer.files)) }}><span className="pic-upload"><Icon name="arrowUp" /></span><strong>{sources.length ? '点击或拖入，继续添加 / 替换' : '拖拽文件到这里，或点击选择'}</strong><small>{mode === 'pdf' ? '单个 PDF · 最多导出 100 页' : 'JPG / PNG / WebP · 最多 40 张'} · 总计 100 MB 以内</small></button>
      <div className="pic-source-list">{sources.map((source, index) => <article key={source.id}>{mode === 'images' ? <img src={source.url} alt={source.file.name} /> : <span className="pic-pdf-symbol">PDF</span>}<div><strong title={source.file.name}>{source.file.name}</strong><small>{mode === 'images' ? `第 ${index + 1} 页 · ` : ''}{formatPdfBytes(source.file.size)}</small></div><div className="pic-order">{mode === 'images' && <><button disabled={locked || index === 0} aria-label={`上移 ${source.file.name}`} onClick={() => move(index, -1)}><Icon name="arrowUp" /></button><button disabled={locked || index === sources.length - 1} aria-label={`下移 ${source.file.name}`} onClick={() => move(index, 1)}><Icon name="arrowDown" /></button></>}<button disabled={locked} aria-label={`移除 ${source.file.name}`} onClick={() => { invalidate(); release(source.url); setSources(sources.filter((item) => item.id !== source.id)) }}><Icon name="close" /></button></div></article>)}</div>
